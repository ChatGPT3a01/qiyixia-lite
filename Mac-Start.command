...
AUTH_RESULT=$("$NODE_BIN" "$UCLAW_DIR/app/auth.mjs" check "$UCLAW_DIR/data/config.env" 2>&1)
AUTH_EXIT=$?
if [ $AUTH_EXIT -eq 2 ]; then
    echo ""
    echo "  [授權已到期] 請聯絡 3a01chatgpt@gmail.com 取得續約碼"
    echo ""
    read -p "  輸入續約碼（無則按 Enter 離開）: " RENEWAL_CODE
    if [ -n "$RENEWAL_CODE" ]; then
        "$NODE_BIN" "$UCLAW_DIR/app/auth.mjs" apply-renewal "$UCLAW_DIR/data/config.env" "$RENEWAL_CODE"
        if [ $? -ne 0 ]; then
            echo "  續約失敗。"
            read -p "按 Enter 離開..."
            exit 1
...
echo -e "  ${GREEN}════════════════════════════════${NC}"
echo -e "  ${GREEN}🦞 起易蝦 is running!${NC}"
