...
echo   -- Chat Platforms --
echo   [3] LINE Bot
echo   [4] Other platforms (Telegram/Discord)
echo.
echo   -- Maintenance --
echo   [5] Diagnostics
...
echo   === LINE Bot Setup ===
echo.
echo   LINE plugin will be installed if needed.
echo   You need your Channel Access Token and Channel Secret.
echo.
echo   Get them at: https://developers.line.biz/console/
echo.
set /p linetoken="  Channel Access Token: "
set /p linesecret="  Channel Secret: "
if "%linetoken%"=="" goto :qq_cancel
if "%linesecret%"=="" goto :qq_cancel
cd /d "%CORE_DIR%"
"%NODE_BIN%" "%OPENCLAW_MJS%" plugins install @openclaw/line
"%NODE_BIN%" "%OPENCLAW_MJS%" config set channels.line.enabled true
"%NODE_BIN%" "%OPENCLAW_MJS%" config set channels.line.channelAccessToken "%linetoken%"
"%NODE_BIN%" "%OPENCLAW_MJS%" config set channels.line.channelSecret "%linesecret%"
"%NODE_BIN%" "%OPENCLAW_MJS%" config set channels.line.dmPolicy pairing
echo.
set /p lineallow="  LINE User ID allowlist (empty to skip): "
if not "%lineallow%"=="" "%NODE_BIN%" "%OPENCLAW_MJS%" config set channels.line.allowFrom "%lineallow%"
echo.
echo   LINE Bot configured! Webhook path defaults to /line/webhook.
...
echo.
echo   [a] Telegram         - International
echo   [c] Discord
echo.
set /p ch_choice="  Choose (a/c, empty to cancel): "
if "%ch_choice%"=="a" (
    echo.
    echo   Telegram setup:
...
if exist "%USERPROFILE%\.uclaw" (
    ...
    echo     Open Settings - Apps - find 起易蝦 - Uninstall
)
pause
goto :menu
