...
show_menu() {
    ...
    echo -e "  ${GREEN}[3]${NC}  接入 LINE Bot（台湾常用）"
    echo -e "  ${GREEN}[4]${NC}  接入其他平台（Telegram/Discord）"
    echo ""
...
do_qq() {
    echo ""
    echo -e "  ${CYAN}${BOLD}━━━ 接入 LINE Bot ━━━${NC}"
    echo ""
    echo -e "  ${GREEN}会先安装 LINE 插件，再写入 LINE Bot 设定。${NC}"
    echo ""
    echo "  获取方式: https://developers.line.biz/console/"
    echo ""
    read -p "  Channel Access Token: " LINE_TOKEN
    read -p "  Channel Secret: " LINE_SECRET
    echo ""

    if [ -z "$LINE_TOKEN" ] || [ -z "$LINE_SECRET" ]; then
        echo -e "  ${YELLOW}已取消${NC}"
        return
    fi

    run_oc plugins install @openclaw/line 2>&1 || true
    run_oc config set channels.line.enabled true 2>&1 || true
    run_oc config set channels.line.channelAccessToken "\"${LINE_TOKEN}\"" 2>&1 || true
    run_oc config set channels.line.channelSecret "\"${LINE_SECRET}\"" 2>&1 || true
    run_oc config set channels.line.dmPolicy "\"pairing\"" 2>&1 || true
    echo ""
    read -p "  你的 LINE User ID（白名单，留空跳过）: " LINE_ALLOW
    if [ -n "$LINE_ALLOW" ]; then
        run_oc config set channels.line.allowFrom "\"${LINE_ALLOW}\"" 2>&1 || true
        echo -e "  ${GREEN}白名单已设置${NC}"
    fi
    echo ""
    echo -e "  ${GREEN}LINE Bot 设定完成！预设 webhook 路径为 /line/webhook。${NC}"
}
...
do_platforms() {
    ...
    echo ""
    echo -e "  ${GREEN}[a]${NC} Telegram         — 海外推荐"
    echo -e "  ${GREEN}[c]${NC} Discord"
    echo ""
    read -p "  选择 (a/c): " -n 1 CH
    echo ""
    echo ""

    case $CH in
        a) echo "  Telegram: 找 @BotFather 创建机器人" ;;
        c) echo "  Discord: 访问 discord.com/developers/applications" ;;
        *) echo "  无效选择" ;;
    esac
    echo ""
